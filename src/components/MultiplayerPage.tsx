        ...
        <>
          ...
          {waiting ? <section className="room-waiting">
            ...
            <aside className="game-sidebar">
              ...
              {[1, 2].map((color) => {
                const player = snapshot!.players.find((p) => p.color === color);
                return <PlayerCard key={`${snapshot!.matchId}-${color}`} name={player?.name ?? "Friend"} color={color === 1 ? "black" : "white"} score={color === 1 ? score.black : score.white} winning={!finished && (color === 1 ? score.black > score.white : score.white > score.black)} active={!finished && snapshot!.turn === color} styleId={player?.styleId ?? "classic"} you={player?.id === m.playerId} opponentLabel="Friend" outcome={!finished ? undefined : draw ? "draw" : snapshot!.result?.winner === color ? "win" : "loss"} reducedMotion={motion} />;
              })}
              ...
              <div className="game-tools"><button className="button" disabled={!roomReady || m.pending || !!finished} onClick={() => setConfirmLeave("surrender")}><Flag size={17} /> Surrender</button><button className="button" onClick={onSettings}><Settings2 size={17} /> Settings</button></div>
