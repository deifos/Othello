        ...
        <>
          ...
          {waiting ? <section className="room-waiting">
            ...
            <aside className="game-sidebar">
              ...
              {[1, 2].map((color) => {
                const player = snapshot!.players.find((p) => p.color === color);
                return <PlayerCard key={`${snapshot!.matchId}-${color}`} name={player?.name ?? "Friend"} color={color === 1 ? "black" : "white"} score={color === 1 ? score.black : score.white} active={!finished && snapshot!.turn === color} styleId={player?.styleId ?? "classic"} you={player?.id === m.playerId} opponentLabel="Friend" outcome={!finished ? undefined : draw ? "draw" : snapshot!.result?.winner === color ? "win" : "loss"} reducedMotion={motion} />;
              })}
