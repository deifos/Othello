...
Choose **Friends** or **Play with Friends**. Create a room, then share its invite
link or six-character code. Both players choose **I'm ready**. Black plays first.
...
swap for the next round. Online undo is not available.
...
winner. Reloading within the grace period restores the same seat and board.
...
It covers PartyKit-only hosting and a Vercel website with PartyKit services,
including the direct practice URL and the allowed-origin setting.

The live app is [flip-buddies.deifos.partykit.dev](https://flip-buddies.deifos.partykit.dev/).
It was deployed and tested with two separate players on 6 September 2026.
...
Friend matches are unranked. They never enter the client-submitted CPU practice
leaderboard. The practice API verifies each CPU move transcript and derives the
...
- `src/game/engine.ts`: pure rules shared by the CPU and the room server.
- `src/game/protocol.ts`: version 2 commands, player assignments, and snapshots.
...
version, match ID, revision, turn, and legal tile. Player identity comes from the
connection's private token, not a claimed color in the move. Only token hashes
are stored; public snapshots include no secrets. Accepted command IDs make
retries idempotent. Input, connection, queue, and request limits are bounded.
